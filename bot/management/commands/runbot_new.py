"""
Management command برای اجرای ربات تلگرام - نسخه بهینه شده
"""
import logging
import os
import django
...
class Command(BaseCommand):
    help = 'اجرای ربات تلگرام - نسخه بهینه شده'
    ...
    def handle(self, *args, **options):
        ...
        application.add_handler(MessageHandler(filters.Regex(f'^{MENU_TRADE}$'), show_trade_menu))
...
async def show_trade_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """نمایش منوی معامله"""
    telegram_id = str(update.effective_user.id)
    is_approved, profile = await UserService.acheck_user_approval_status(telegram_id)
...
async def trade_product_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # استخراج product_code
    callback_data = query.data
    product_code = callback_data.replace(CALLBACK_TRADE_PRODUCT_PREFIX, "").split("_")[0]
    
    # بررسی اگر خرید/فروش هم انتخاب شده
    if f"_{CALLBACK_ACTION_BUY}" in callback_data:
        action = "buy"
    elif f"_{CALLBACK_ACTION_SELL}" in callback_data:
        action = "sell"
    else:
        # فقط محصول انتخاب شده، باید خرید/فروش انتخاب شود
        try:
            # نمایش قیمت و گزینه خرید/فروش
            product_code_map = {
    ...
    # خرید یا فروش مستقیم انتخاب شده
    context.user_data['action'] = action
    product_code = callback_data.replace(CALLBACK_TRADE_PRODUCT_PREFIX, "").replace(f"_{CALLBACK_ACTION_BUY}", "").replace(f"_{CALLBACK_ACTION_SELL}", "")
...
async def trade_method_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if method == "gram":
        prompt = f"لطفاً مقدار را به *گرم* یا *عدد* وارد کنید:\n\nمثال: 2.5"
    else:
        prompt = f"لطفاً مبلغ را به *ریال* وارد کنید:\n\nمثال: 5000000"
...
async def trade_amount_entered(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if amount is None:
        await update.message.reply_text(
            "❌ مقدار نامعتبر. لطفاً یک عدد معتبر وارد کنید."
        )
        return ENTERING_AMOUNT
    ...
    invoice = (
        ...
        f"💵 قیمت: *{format_number(price)}* ریال\n"
...
async def trade_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        logger.info(f"سفارش {action_text}: {order.id} - {profile.phone_number}")
...
def get_trade_conversation_handler() -> ConversationHandler:
    ...
    return ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex(f'^{MENU_TRADE}$'), show_trade_menu)
        ],
        states={
            SELECTING_PRODUCT: [
                CallbackQueryHandler(trade_product_selected, pattern=f'^{CALLBACK_TRADE_PRODUCT_PREFIX}'),
            ],
            SELECTING_ACTION: [
                CallbackQueryHandler(trade_action_selected, pattern=f'^{CALLBACK_TRADE_PRODUCT_PREFIX}'),
            ],
            SELECTING_METHOD: [
