...
- درخواست وجه و انتقال بین کاربران
- مدل تراکنش‌های مالی
...
**دلیل Frozen Balance**:
- هنگام ثبت درخواست برداشت یا انتقال، موجودی مسدود می‌شود
...
- related_user (ForeignKey به Profile - اختیاری، برای انتقال بین کاربران)
...
### 5️⃣ مدل `TransferRequest` (درخواست انتقال وجه)
...
- id (Primary Key)
- request_number (شماره یونیک درخواست)
- sender_profile (ForeignKey به Profile - فرستنده)
- receiver_profile (ForeignKey به Profile - گیرنده)
- receiver_phone (شماره تلفن گیرنده - برای جستجو)
- currency_type (نوع ارز)
- amount (مقدار)
- status (وضعیت: PENDING, COMPLETED, CANCELLED)
- sender_transaction (ForeignKey به Transaction)
- receiver_transaction (ForeignKey به Transaction)
- description (توضیحات)
- created_at
- completed_at
```
...
```
search_user_by_phone(phone_number)
    ↳ جستجوی کاربر بر اساس شماره تلفن
    ↳ بررسی وجود و تایید شده بودن کاربر
    ↳ Return: Profile یا None

create_transfer_request(sender_profile, receiver_phone, currency_type, amount, description)
    ↳ ایجاد درخواست انتقال وجه
    ↳ جستجوی گیرنده
    ↳ بررسی کفایت موجودی فرستنده
    ↳ مسدود کردن موجودی فرستنده
    ↳ ایجاد دو تراکنش: TRANSFER_SEND و TRANSFER_RECEIVE
    ↳ ایجاد TransferRequest
    ↳ تایید خودکار (بدون نیاز به ادمین) یا منتظر تایید
    ↳ استفاده از @transaction.atomic()

complete_transfer(transfer_request_id)
    ↳ تکمیل انتقال
    ↳ کسر از frozen_balance فرستنده
    ↳ افزودن به balance گیرنده
    ↳ تکمیل هر دو تراکنش
    ↳ ارسال نوتیفیکیشن به هر دو کاربر
    ↳ استفاده از @transaction.atomic()

cancel_transfer(transfer_request_id, reason)
    ↳ لغو انتقال
    ↳ آزاد کردن موجودی فرستنده
    ↳ لغو هر دو تراکنش
```
...
def get_wallet_menu_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔄 انتقال وجه", callback_data="wallet_transfer")],
...
4. **انتقال وجه**:
   - تایید هویت گیرنده
   - بررسی اینکه فرستنده و گیرنده یکی نباشند
   - مسدود کردن موجودی تا تکمیل انتقال
   - Rollback خودکار در صورت خطا

### Rate Limiting:
- محدودیت تعداد درخواست‌های واریز/برداشت در روز
- محدودیت تعداد انتقال‌های وجه در ساعت
...
```
فرستنده: کلیک "انتقال وجه"
    ↓
انتخاب نوع ارز
    ↓
ورود شماره تلفن گیرنده
    ↓
جستجوی کاربر در سیستم
    ↓ (پیدا شد)
...
بررسی کفایت موجودی فرستنده
    ↓ (کافی است)
(اختیاری) ورود توضیحات
    ↓
نمایش خلاصه و تایید نهایی
    ↓
مسدود کردن موجودی فرستنده
    ↓
ایجاد TransferRequest + 2 Transaction
    ↓
انجام انتقال (با transaction.atomic)
    ↓
کسر از موجودی فرستنده
    ↓
افزودن به موجودی گیرنده
    ↓
تکمیل هر دو Transaction
    ↓
ارسال نوتیفیکیشن به هر دو کاربر
```
...
SELECTING_TRANSFER_CURRENCY = "selecting_transfer_currency"
ENTERING_RECEIVER_PHONE = "entering_receiver_phone"
ENTERING_TRANSFER_AMOUNT = "entering_transfer_amount"
ENTERING_TRANSFER_DESCRIPTION = "entering_transfer_description"
...
CALLBACK_WALLET_TRANSFER = "wallet_transfer"
